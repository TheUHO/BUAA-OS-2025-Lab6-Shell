...
void mips_detect_memory(u_int _memsize) {
	...
	npage = memsize / PAGE_SIZE; // PAGE_SIZE 定义在mmu.h中
...
void page_init(void) {
	...
	u_long size = PPN(PADDR(freemem));
	struct Page *pageptr = pages;
	for (u_long i = 0; i < size; i++, pageptr++) {
		pageptr->pp_ref = 1;
	}
	/* Step 4: Mark the other memory as free. */
	/* Exercise 2.3: Your code here. (4/4) */
	for (u_long i = size; i < npage; i++, pageptr++) {
		pageptr->pp_ref = 0;
...
int page_alloc(struct Page **new) {
	...
	if (LIST_EMPTY(&page_free_list)) {
		return -4; // E_NO_MEM defined in include/error.h
	}
